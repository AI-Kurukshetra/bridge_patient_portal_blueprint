...
import { buildFhirProcedure } from "@/lib/fhir/resources";
...
export async function GET(request: Request) {
  ...
  const resources = filterByLastUpdated(context.procedures, lastUpdated, (item) => item.performed_at);
  const { count, offset } = parsePagination(url);
  const filtered = paginate(resources, count, offset);

  return fhirJson(makeBundle(filtered.map((item) => ({ resource: buildFhirProcedure(item, context.patient.id) })), resources.length));
}